...
export function parseCompletionTime(status: string | null | undefined): Date | null {
...
export function getEffectiveState(
	...
	if (state === 'available' && wasRecentlyUsed(status, currentTime)) {
...
export function canTrackMachine(
	state: MachineState | string,
	status: string | null | undefined,
	currentTime: number
): boolean {
	const effectiveState = getEffectiveState(state, status, currentTime);
	return effectiveState !== 'available' && effectiveState !== 'recently_used';
}
...
export function wasRecentlyUsed(status: string | null | undefined, currentTime: number): boolean {
