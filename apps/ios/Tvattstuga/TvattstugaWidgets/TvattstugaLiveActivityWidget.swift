    ...
    var body: some WidgetConfiguration {
        ActivityConfiguration(for: MachineLiveActivityAttributes.self) { context in
            ...
            DynamicIsland {
                ...
                // Keep the icon intrinsic in its own slot.
                WasherActivityIcon(size: 16)
            } compactTrailing: {
                // Constrain the trailing slot so the time doesn't hug the far right.
                // Adjust maxWidth as needed; ~60–80 works well for "59:59".
                CountdownText(endDate: context.state.endDate)
                    .font(.caption.monospacedDigit().weight(.semibold))
                    .frame(maxWidth: 50, alignment: .trailing) // tweak 70–84 to taste
